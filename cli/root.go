// Package cli builds the mathse command tree on top of the mathse library.
package cli
...
func Root() *cobra.Command {
	app := &App{cfg: mathse.DefaultConfig()}

	root := &cobra.Command{
		Use:   "mathse",
		Short: "Search Math Stack Exchange questions",
		Long: `mathse reads Math Stack Exchange through the public Stack Exchange API.
No API key is required for up to 300 requests per day. It returns records
as table, JSON, JSONL, CSV, TSV, or URLs.

mathse is an independent tool and is not affiliated with Stack Exchange.`,
		SilenceUsage:  true,
		SilenceErrors: true,
...
func (a *App) setup() error {
	...
	a.client = mathse.NewClient(a.cfg)
	return nil
}
