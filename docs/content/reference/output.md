...
describes what users get. Pick a format with `-o`, or let mathse choose:
a table when writing to a terminal, JSONL when piped.

## Formats

```bash
mathse <command> -o table   # aligned columns for reading
mathse <command> -o jsonl   # one JSON object per line, for piping
mathse <command> -o json    # a single JSON array
mathse <command> -o csv     # spreadsheet friendly
mathse <command> -o tsv     # tab-separated
mathse <command> -o url     # just the URL column
mathse <command> -o raw     # the underlying bytes, unformatted
```
...
```bash
mathse <command> --fields id,title,url
```
...
```bash
mathse <command> --template '{{.URL}} {{.Title}}'
```
...
```bash
mathse <command>            # a table, because this is a terminal
mathse <command> | wc -l    # JSONL, because this is a pipe
```
